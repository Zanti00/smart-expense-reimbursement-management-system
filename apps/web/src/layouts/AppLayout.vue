...
import {
  ...
  ClipboardList, FileBarChart2, Bell, LogOut, Menu, X, ChevronRight, FlaskConical, FileCheck
...
<template>
  <div class="flex h-screen overflow-hidden bg-clinical font-sans select-none">
    ...
    >
      <!-- Branding Module -->
      <div class="flex items-center gap-3 px-4 py-6 border-b border-white/10 min-h-[70px]">
        <div class="w-8 h-8 border border-white/30 flex items-center justify-center bg-white/10 flex-shrink-0">
          <FlaskConical class="w-4 h-4 text-white" />
        </div>
        <Transition name="fade">
          <div v-if="sidebarOpen" class="overflow-hidden whitespace-nowrap">
            <p class="text-white font-bold text-base tracking-widest uppercase leading-none">SERMS</p>
          </div>
        </Transition>
      </div>
      ...
      <nav class="flex-1 px-0 py-4 space-y-0 overflow-y-auto scrollbar-thin">
        <template v-for="(link, i) in navLinks" :key="i">
          ...
          <div v-else-if="link.header" class="px-6 py-2">
            <Transition name="fade">
              <span v-if="sidebarOpen" class="text-[11px] font-bold uppercase tracking-wider text-primary-300 opacity-80 flex items-center gap-2">
                <div class="w-1.5 h-1.5 bg-primary-400 opacity-50" /> {{ link.header }}
              </span>
            </Transition>
          </div>
          <RouterLink
            ...
            @click="mobileOpen = false"
          >
            <component :is="link.icon" :class="['w-4 h-4 flex-shrink-0', isActive(link.to) ? 'text-white' : 'text-slate-400']" />
            <Transition name="fade">
      ...
      <div class="border-t border-white/10 bg-black/10 p-4">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 border border-white/20 flex items-center justify-center flex-shrink-0 text-white text-[10px] font-bold font-mono">
            {{ auth.user?.avatar }}
          </div>
          <Transition name="fade">
            <div v-if="sidebarOpen" class="flex-1 min-w-0">
              <p class="text-white text-xs font-bold uppercase truncate tracking-wide">{{ auth.user?.name }}</p>
              <div class="flex items-center gap-1.5 mt-0.5">
                <div class="w-1.5 h-1.5 bg-success rounded-full" />
                <p class="text-white/70 text-[10px] font-bold uppercase tracking-wider">{{ auth.user?.role }}</p>
              </div>
            </div>
          </Transition>
          <button
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
      <!-- Console Header -->
      <header class="flex items-center gap-4 px-4 lg:px-6 py-3 bg-white border-b border-slate-200 z-10 min-h-[70px]">
        <!-- Mobile Trigger -->
        ...
        <!-- System Alerts -->
        <button id="notif-bell" class="btn btn-secondary !p-2 relative" @click="notifOpen = !notifOpen">
          <Bell class="w-4 h-4" />
          <span
            v-if="unreadCount > 0"
      ...
      <!-- Active Workspace -->
      <main class="flex-1 overflow-y-auto scrollbar-thin p-6 animate-fade-in bg-clinical/50">
...
.page-enter-active, .page-leave-active { transition: opacity 0.15s linear; }
...
<style scoped>
.fade-text-enter-active, .fade-text-leave-active { transition: opacity 0.15s ease, width 0.3s ease; }
