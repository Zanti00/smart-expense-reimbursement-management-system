...
export const useReceiptStore = defineStore('receipts', () => {
  ...
  async function fetchCategories() {
    try {
      ...
      if (response.ok) {
        ...
        useNotificationStore().error(`Categories API returned ${response.status}: ${response.statusText}`)
      }
    } catch (e) {
  ...
  async function uploadReceipt(file, metadata) {
    ...
    if (metadata.vendor_name) formData.append('vendor_name', metadata.vendor_name)
    if (metadata.transaction_date) formData.append('transaction_date', metadata.transaction_date)
    if (metadata.total_amount != null) formData.append('total_amount', metadata.total_amount)
    if (metadata.vat_amount != null) formData.append('vat_amount', metadata.vat_amount)
    if (metadata.tin) formData.append('tin', metadata.tin)
    if (metadata.invoice_number) formData.append('invoice_number', metadata.invoice_number)
    if (metadata.vat_classification) formData.append('vat_classification', metadata.vat_classification)
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.message || `Failed to save receipt (Status ${response.status})`)
      }
  ...
  async function softDelete(id) {
    ...
    try {
      ...
      const response = await fetch(`/api/serms/reimbursements/receipts/${rx.dbId}`, {
        method: 'DELETE',
        headers,
        credentials: 'include'
      })
      ...
      if (!response.ok) {
        const errData = await response.json().catch(() => ({}))
        throw new Error(errData.message || `Failed to delete receipt (Status ${response.status})`)
      }
