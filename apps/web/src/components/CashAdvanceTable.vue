    ...
    <div class="flex flex-col gap-1 border-b border-slate-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-0.5 text-xs text-slate-400">
          {{ isAdmin ? 'Administrative review queue' : 'Your cash advance records' }}
        </p>
      </div>
      <span class="kpi-label text-slate-400">Showing {{ rows.length }} records</span>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full border-collapse text-left" :class="isAdmin ? 'min-w-[1180px]' : 'min-w-[880px]'">
        <thead>
          <tr class="border-b border-slate-200 bg-slate-50">
            <th class="px-5 py-4 text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">ID</th>
            <th v-if="isAdmin" class="px-5 py-4 text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">File Description</th>
            ...
            <th v-if="isAdmin" class="px-5 py-4 text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">Due Date</th>
            <th class="px-5 py-4 text-right text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">Amount</th>
            <th v-if="isAdmin" class="px-5 py-4 text-right text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">Outstanding</th>
            <th v-if="isAdmin" class="px-5 py-4 text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">User</th>
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="row in rows" :key="row.id" class="group whitespace-nowrap transition-colors duration-200 ease-out hover:bg-slate-50/80">
            <td class="px-5 py-5 font-mono text-sm font-bold text-slate-900">{{ row.id }}</td>
            <td v-if="isAdmin" class="max-w-[170px] px-5 py-5 text-sm text-slate-500"><span class="block truncate">{{ row.fileDescription }}</span></td>
            ...
            <td v-if="isAdmin" class="px-5 py-5 text-sm text-slate-500">{{ row.dueDate }}</td>
            <td class="px-5 py-5 text-right text-sm font-bold text-primary">{{ formatPeso(row.amount) }}</td>
            <td v-if="isAdmin" class="px-5 py-5 text-right text-sm font-semibold text-slate-600">{{ formatPeso(row.outstanding) }}</td>
            <td v-if="isAdmin" class="px-5 py-5">
              <div class="flex items-center gap-2">
                <span class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-[10px] font-bold text-primary">{{ row.initials }}</span>
